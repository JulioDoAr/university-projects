...
import com.circuit.comparator.CircuitComparatorByComplexity;
import com.logger.ConsoleOutputOperation;
...
class CampeonatoPrematuroTest {
	...
	void test() {
		...
		log.addOutputOperation(new FileOutputOperation("src/test/resources/output/SalidaDatosCampeonatoPrematuro.txt"));
		log.startWrite();
		log.write(
				"*********************************************************************************************************");
		log.write(
				"****ESTA SIMULACIÓN CONCLUYE ANTES DE FINALIZAR EL CAMPEONATO CON UN ÚNICO PILOTO SIN DESCALIFICAR****");
		log.write(
				"*********************************************************************************************************\n");
		initData();
		log.finishWrite();
	...
	private void initData() {
		...
		organization.setDropoutsLimit(1);
		organization.setPilotsLimit(3);
		// Circuitos ordenados de forma descendente de acuerdo a su complejidad
		organization.setCircuitComparator(new CircuitComparatorByComplexity().reversed());
		...
		// empate por nombre);
		Team citroen = new Team("Citroen", new PilotComparatorByPointsDexterityName(),
